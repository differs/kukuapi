...
const roleLabel = computed(() => {
  return user.value?.role === 'admin' ? t('profile.administrator') : t('profile.user')
})
...
const userInitials = computed(() => {
  ...
  return roleLabel.value.slice(0, 1).toUpperCase()
})

const displayName = computed(() => {
  if (!user.value) return ''
  return user.value.username || roleLabel.value
})
