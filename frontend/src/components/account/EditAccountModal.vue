  ...
  >
    ...
    >
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.openai.upstreamWireAPI') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.openai.upstreamWireAPIDesc') }}
            </p>
          </div>
          <div class="w-52">
            <Select v-model="openAIUpstreamWireAPI" :options="openAIUpstreamWireAPIOptions" />
          </div>
        </div>
      </div>
      ...
      <div
        v-if="account?.platform === 'openai' && (account?.type === 'oauth' || (account?.type === 'apikey' && !isOpenAIChatCompletionsUpstream))"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between">
          <div>
...
// OpenAI 自动透传开关（OAuth/API Key）
const OPENAI_UPSTREAM_WIRE_API_RESPONSES = 'responses'
const OPENAI_UPSTREAM_WIRE_API_CHAT_COMPLETIONS = 'chat_completions'
const openAIUpstreamWireAPI = ref<typeof OPENAI_UPSTREAM_WIRE_API_RESPONSES | typeof OPENAI_UPSTREAM_WIRE_API_CHAT_COMPLETIONS>(OPENAI_UPSTREAM_WIRE_API_RESPONSES)
...
const openAIUpstreamWireAPIOptions = computed(() => [
  { value: OPENAI_UPSTREAM_WIRE_API_RESPONSES, label: t('admin.accounts.openai.upstreamWireAPIResponses') },
  { value: OPENAI_UPSTREAM_WIRE_API_CHAT_COMPLETIONS, label: t('admin.accounts.openai.upstreamWireAPIChatCompletions') }
])
const isOpenAIChatCompletionsUpstream = computed(() =>
  props.account?.platform === 'openai' && props.account?.type === 'apikey' && openAIUpstreamWireAPI.value === OPENAI_UPSTREAM_WIRE_API_CHAT_COMPLETIONS
)
...
const canUseOpenAIPassthrough = computed(() =>
  props.account?.platform === 'openai' && (props.account?.type !== 'apikey' || !isOpenAIChatCompletionsUpstream.value)
)
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  openAIUpstreamWireAPI.value = OPENAI_UPSTREAM_WIRE_API_RESPONSES
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'apikey')) {
    openAIUpstreamWireAPI.value = extra?.openai_upstream_wire_api === OPENAI_UPSTREAM_WIRE_API_CHAT_COMPLETIONS
      ? OPENAI_UPSTREAM_WIRE_API_CHAT_COMPLETIONS
      : OPENAI_UPSTREAM_WIRE_API_RESPONSES
  ...
  try {
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'apikey')) {
      ...
      if (props.account.type === 'oauth') {
        ...
        delete newExtra.openai_upstream_wire_api
      } else if (props.account.type === 'apikey') {
        if (openAIUpstreamWireAPI.value === OPENAI_UPSTREAM_WIRE_API_CHAT_COMPLETIONS) {
          newExtra.openai_upstream_wire_api = OPENAI_UPSTREAM_WIRE_API_CHAT_COMPLETIONS
      ...
      if (canUseOpenAIPassthrough.value && openaiPassthroughEnabled.value) {
